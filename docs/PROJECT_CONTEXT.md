...
2. **回測執行階段**：
   - 從資料庫讀取資料 → `BacktestEngine.run_backtest()` → 逐日迭代
   - 每日：取得景氣燈號 → 策略產生訂單 → 執行交易 → 計算持倉價值
   - 驗證檢查：記錄信號、訂單、持倉快照 → 回測結束後驗證

3. **結果輸出階段**：
   - 計算績效指標 → 輸出CSV檔案 → 生成驗證報告
...
**檔案**：`backtesting/backtest_engine.py`
...
- `_execute_order(order, date, price_dict)`: 執行單筆訂單（買進或賣出）
...
**交易執行邏輯**：
- **買進信號**：當偵測到從非藍燈變成藍燈時，在發布日期後的5個交易日分批買進
- **賣出信號**：當偵測到從非紅燈變成紅燈時，在隔月的最後5個交易日分批賣出

### 分批執行邏輯

所有買賣訂單都會分成5天執行，以降低市場衝擊：
- **買進**：從發布日期開始，連續5個交易日分批買進（每天約20%）
- **賣出**：從隔月最後5個交易日開始，分批賣出（如當月交易日不足5天，會延伸到次月）
